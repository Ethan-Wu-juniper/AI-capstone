...
def find_spacious(mapStat,playerID):
    Map = copy(mapStat)
    Sheep = (Map > 0).astype('int32')
    for i in range(12):
        for j in range(12):
    ...
    for i in range(12):
        for j in range(12):
            Map_cpy = copy(Map)
            Sheep_cpy = copy(Sheep)
            distance = 0
            if Map_cpy[i][j] != 0:
                continue
            Map_cpy[i][j] = playerID
            Sheep_cpy[i][j] = 16
            move = gr.checkRemainMove(playerID,Map_cpy,Sheep_cpy)
            if len(move) != 5:
                continue
            # print(move)
            for m in move:
                new_x, new_y = gr.sheepMove([i,j], m[1], Map_cpy)
                # print(new_x,new_y)
                if m[1] == 3 or m[1] == 4:
                    distance += abs(new_x - i)
                else:
                    distance += abs(new_y - j)
            if distance > max_d:
                max_d = distance
                valid_pos = []
            valid_pos.append([i,j])
    init_pos = random.choice(valid_pos)
    return init_pos

def InitPos(mapStat,playerID):
    # init_pos = find_init(mapStat,playerID)
    init_pos = find_spacious(mapStat,playerID)
